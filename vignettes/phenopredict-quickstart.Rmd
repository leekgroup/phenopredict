...
Quick start to using to `r ('phenopredict')`
================================================
...
library("roxygen2")
library("knitr")
document("/users/sellis/packagename")
```
# Load data

```{r 'load-data', message = FALSE, warning = FALSE}
source("/dcs01/ajaffe/GTEX/Leek/coverageMatrix/simpleLoad/gtexLoad.R")
load("/users/sellis/phenopredict_old/data/sysdata.rda")
load("/users/sellis/phenopredict_old/data/extdata.R")   
#cm, cm_new, regiondata, pheno to run example
```
...
predictor<-build_predictor(inputdata=inputdata ,phenodata=pheno, phenotype="Sex", covariates=c("AGE","BMI"),type="factor", numRegions=10)
```
# Test predictor

If you want to test the accuracy of your build predictor on data in which the phenotype is known, use predictions_test.
...
predictions<-predict_pheno(expression= test_data, phenodata=pheno, phenotype="Sex",predictordata = predictor)
```
